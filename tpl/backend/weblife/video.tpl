<div id="sectionTitle"><{$smarty.const.VIDEOS}></div>
...
<{/if}>
    <div class="breadcrumb">
        <{$smarty.const.ADMIN_PATH}> <{$arrPageData.path_arrow}> <a href="<{$arrPageData.admin_url}>" title=""><{$smarty.const.VIDEOS}></a>
<!-- ++++++++++ Start BreadCrumb +++++++++++++++++++++++++++++++++++++++++++ -->
...
<form method="post" action="<{$arrPageData.current_url|cat:$arrPageData.filter_url|cat:"&task="|cat:$arrPageData.task}><{if $arrPageData.itemID>0}><{''|cat:"&itemID="|cat:$arrPageData.itemID}><{/if}>" name="<{$arrPageData.task}>Form" onsubmit="return formCheck(this);" enctype="multipart/form-data">
    ...
    <table width="100%" border="0" cellspacing="1" cellpadding="0" class="list">
        <tr>
            <td id="headb" align="left">Category</td>
        </tr>
        <tr>
...
<{/if}>
<{sectionelse}>
                    <option value="<{$arrPageData.arrParent.id}>"> &nbsp; <{$arrPageData.arrParent.title}> &nbsp; [items: <{if isset($arCidCntItems[$arrPageData.arrParent.id])}><{$arCidCntItems[$arrPageData.arrParent.id]}><{else}>0<{/if}>] &nbsp; </option>
<{/section}>
                </select>
            </td>
        </tr>
    ...
    <table width="100%" border="0" cellspacing="1" cellpadding="0" class="list">
        ...
        <tr id="toogleImageBox">
            <td id="body2" width="85%">
                ...
                <{if !empty($item.image)}>Файл: <a href="<{$arrPageData.files_url|cat:$item.image}>" onclick="return hs.expand (this, { })" class="highslide" title="<{$item.title}>"><{$item.image}></a><{/if}>
            </td>
            <td id="body2" align="center">
    ...
    <table width="100%" border="0" cellspacing="1" cellpadding="0" class="list">
        ...
        <tr id="toogleFileBox">
            <td id="body2" width="95%">
                ...
                <{if !empty($item.filename)}>Файл: <a href="/flash/JWPlayer/player.swf" onclick="return hs.htmlExpand(this,
                    {
                        useBox:'true',
...
<{/if}>
                        ...
                        swfOptions: {
                            ...
                            flashvars: { file: '&file=<{$arrPageData.files_url|cat:$item.filename}>&backcolor=000000&frontcolor=ffffff&lightcolor=555555&screencolor=000000&screencolor=000000', stretching:'fill',<{if !empty($item.image)}> image:'<{$arrPageData.files_url|cat:$item.image}>',  autostart:'false'<{else}> autostart:'true'<{/if}> },
...
<{/if}>
            <td id="headb" align="center" width="40"><{$smarty.const.HEAD_IMAGE}></td>
            <td id="headb" align="center" width="40"><{$smarty.const.HEAD_FILENAME}></td>
...
<{if !empty($items[i].image)}>
                <a href="<{$arrPageData.files_url|cat:$items[i].image}>" onclick="return hs.expand (this, { })" class="highslide" title="<{$items[i].title}>"><{$items[i].image}></a>
<{else}>-<{/if}>
            </td>
            <td id="<{$items[i].idb}>" align="center">
...
<{/if}>
                        ...
                        swfOptions: {
                            version: '9.0.0',
                            expressInstallSwfurl: '/flash/expressInstall.swf',
                            flashvars: { file: '&file=<{$arrPageData.files_url|cat:$items[i].filename}>&backcolor=000000&frontcolor=ffffff&lightcolor=555555&screencolor=000000&screencolor=000000', stretching:'fill',<{if !empty($items[i].image)}> image:'<{$arrPageData.files_url|cat:$items[i].image}>',  autostart:'false'<{else}> autostart:'true'<{/if}> },
                            params: { allowscriptaccess: 'always', allowfullscreen: 'true', quality: 'high' },
...
<{if $items|@count>1}>
        <tr>
            <td id="body1" colspan="10" style="border:0;" align="center">
