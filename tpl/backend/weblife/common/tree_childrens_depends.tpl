...
<{section name=i loop=$arrChildrens}>
            <option value="<{$arrChildrens[i].id}>"<{if in_array($arrChildrens[i].id,$dependIDS)}>  selected<{/if}>><{$arrChildrens[i].margin}><{$arrChildrens[i].title}> &nbsp; ( <{if $arrChildrens[i].active==0}>неактивна, <{/if}><{$arrChildrens[i].menutitle|lower}> ) &nbsp; </option>
