<script type="text/javascript">
    $(document).ready(function() {
        ...
        var scriptURL = '/interactive/ajax.php?zone=admin&action=ajaxGalleryFilesUpload&<{$smarty.const.WLCMS_SESSION_NAME}>=<{$sessionID}>&PID=<{$arrPageData.pid}>&module=<{$arrPageData.pmodule}>&file_prefix=<{$arrPageData.userfileprefix}>';
        $('#userFile').uploadify({
            'queueID'        : 'fileQueue',
...
<div class="row-fluid">
	...
	<div class="controls controls-row">
	    <form id="galleryFilesUpload" action="<{$arrPageData.current_url|cat:$arrPageData.filter_url|cat:'&task=editItems'}>" method="POST">
...
<{section name=i loop=$items}>
			<tr>
			    ...
			    <td>
				<textarea name="arItems[<{$items[i].id}>][descr]" rows="3"><{if !empty($items[i].descr)}><{$items[i].descr}><{/if}></textarea>
			    </td>
			    <td class="center">
<{if $items[i].active==1}>
...
<{/if}>	
	<div class="form-actions">
	    <a class="btn btn-primary" href="javascript:void(0)" onclick="$('form#galleryFilesUpload').submit();" title="<{$smarty.const.BUTTON_RELOAD}>"><{$smarty.const.BUTTON_RELOAD}></a>&nbsp;&nbsp;
