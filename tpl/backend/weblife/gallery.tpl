...
<{else}>
	...
	<div class="control-group">
		...
		<select name="cid" onchange="$('form[name=filterForm]').submit();">
    ...
    <{if !empty($categoryTree[i].childrens)}>
    <!-- ++++++++++ Start Tree Childrens +++++++++++++++++++++++++++++++++++++++ -->
		    <{include file='tree_childrens.tpl' dependID=0 arrChildrens=$categoryTree[i].childrens}>
    <!-- ++++++++++ End Tree Childrens +++++++++++++++++++++++++++++++++++++++++ -->
    <{/if}>
    <{/section}>
		</select>
...
<div class="row-fluid">
	...
	<table class="table table-bordered table-hover admin-list tinytable">
		...
		<tr>
		    ...
		    <th align="left"><{$smarty.const.HEAD_NAME}></th>
		    <th class="center" width="30"><{$smarty.const.HEAD_SORT}></th>
		    <th class="center" width="95"><{$smarty.const.HEAD_DATE_ADDED}></th>
    ...
    <{section name=i loop=$items}>
		<tr>
		   ...
		   <td id="<{$items[i].idb}>" class="center"><{$items[i].created|date_format:"%d.%m.%y %H:%M:%S"}></td>
		   <td id="<{$items[i].idb}>" class="center">
    <{if $items[i].active==1}>
