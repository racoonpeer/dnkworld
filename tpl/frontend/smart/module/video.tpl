                    ...
                    <a id="hs-videoid-<{$items[i].id}>" href="/flash/JWPlayer/player.swf" onclick="return hs.htmlExpand(this,
                        {   objectType: 'swf',
                            swfOptions: {
                                ...
                                flashvars: { file: '&file=<{$items[i].filename}>&backcolor=000000&frontcolor=ffffff&lightcolor=555555&screencolor=000000&screencolor=000000', skin:'/flash/JWPlayer/glow.zip', stretching:'fill',<{if !empty($items[i].image)}> image:'<{$items[i].image}>',  autostart:'false'<{else}> autostart:'true'<{/if}> },
...
<{if $arrModules.video.id!=$arrPageData.catid}>
        <div class="backurl-link">
            <a href="<{include file='core/href.tpl' arItem=$arrModules.video}>"><{$smarty.const.URL_NEWS_BACK}></a>
        </div>
<{/if}>
