...
if(!$objUserInfo->logined){ Redirect($UrlWL->buildItemUrl($arrModules['register'])); }
//Check if user logined. if no - redirect to user view page
if($itemID && $action=='edit' && !$objUserInfo->logined){ Redirect($UrlWL->buildItemUrl($arCategory, 1, array('seo_path'=>UrlWL::USER_SEOPREFIX.$itemID))); }
// User verification
if($itemID && $action=='edit' && $objUserInfo->logined && $itemID!=$objUserInfo->id && $objUserInfo->type!='Administrator' ){ $itemID = $objUserInfo->id; }
// Edit and View User Data Limit For Registered User
if((!$itemID || $itemID!=$objUserInfo->id) && !in_array($objUserInfo->type, array('Administrator', 'User'))){ Redirect($UrlWL->buildItemUrl($arCategory, 1, array('seo_path'=>UrlWL::USER_SEOPREFIX.$objUserInfo->id))); }
// ////////// END OPERATION MANIPULATION WITH SESSION VARIABLE \\\\\\\\\\\\\\\\\
# ##############################################################################
...
if($itemID && $action=='edit' && !empty($_POST)) {
    ...
    if ($Validator->foundErrors()) {
        ...
        if($result){
            ...
            Redirect($UrlWL->buildItemUrl($arCategory, 1, array('id'=>$itemID, 'seo_path'=>UrlWL::USER_SEOPREFIX.$itemID)));
        } else {
...
// List Items
    ...
    $where = '  WHERE t.`active`=1'.((!$objUserInfo->logined || $objUserInfo->type!='Administrator') ? ' AND t.`isnew`=0 AND t.`type`="User" ' : '');
