...
$arrPageData['arrParent']     = getItemRow(GALLERY_TABLE, '*', 'WHERE id='.$pid);
$arrPageData['headTitle']     = GALLERY.$arrPageData['seo_separator'].ADMIN_AJAX_MODE;
...
if($itemID && $task=='deleteItem') {
    unlinkImage($itemID, GALLERYFILES_TABLE, $arrPageData['files_path'], $arrPageData['files_params'], false, 'filename');
    //deleteFileFromDB($itemID, USERFILES_TABLE, 'filename', 'WHERE `id`='.$itemID, $arrPageData['files_path'], false);
    $result = deleteRecords(GALLERYFILES_TABLE, ' WHERE id='.$itemID);
    if(!$result) {
...
elseif($itemID && $task=='publishItem' && isset($_GET['status'])) {
    $result = updateRecords(GALLERYFILES_TABLE, "`active`='{$_GET['status']}'", 'WHERE `id`='.$itemID);
    if($result===false) {
...
// Edit items
elseif (!empty($_POST) && $task=='editItems') {
    if(!empty($_POST['arItems'])) {
	foreach ($_POST['arItems'] as $key=>$value) {
	    $arPostData = array(
		    'descr' => $_POST['arItems'][$key]['descr']
	    );
	    $DB->postToDB($arPostData, GALLERYFILES_TABLE, 'WHERE `id`='.$key, array(), 'update');
	}
    }
}
...
$arrPageData['total_items'] = intval(getValueFromDB(GALLERYFILES_TABLE." t", 'COUNT(*)', $where, 'count'));
...
$query = "SELECT * FROM `".GALLERYFILES_TABLE."` t $where $order $limit";
...
$arrPageData['user_can_upload'] = $arrPageData['userfilesmax']-intval(getValueFromDB(GALLERYFILES_TABLE, 'COUNT(*)', 'WHERE `pid`='.$pid, 'count'));
...
/*
 * 
DROP TABLE IF EXISTS `galleryfiles`;
CREATE TABLE IF NOT EXISTS `galleryfiles` (
