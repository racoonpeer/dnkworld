...
$arrPageData['arrParent']     = getItemRow(NEWS_TABLE, '*', 'WHERE id='.$pid);
$arrPageData['headTitle']     = NEWS.$arrPageData['seo_separator'].ADMIN_AJAX_MODE;
...
if($itemID && $task=='deleteItem') {
    unlinkImage($itemID, NEWSFILES_TABLE, $arrPageData['files_path'], $arrPageData['files_params'], false, 'filename');
    //deleteFileFromDB($itemID, USERFILES_TABLE, 'filename', 'WHERE `id`='.$itemID, $arrPageData['files_path'], false);
    $result = deleteRecords(NEWSFILES_TABLE, ' WHERE id='.$itemID);
    if(!$result) {
...
elseif($itemID && $task=='publishItem' && isset($_GET['status'])) {
    $result = updateRecords(NEWSFILES_TABLE, "`active`='{$_GET['status']}'", 'WHERE `id`='.$itemID);
    if($result===false) {
...
$arrPageData['total_items'] = intval(getValueFromDB(NEWSFILES_TABLE." t", 'COUNT(*)', $where, 'count'));
...
$query = "SELECT * FROM `".NEWSFILES_TABLE."` t $where $order $limit";
...
$arrPageData['user_can_upload'] = $arrPageData['userfilesmax']-intval(getValueFromDB(NEWSFILES_TABLE, 'COUNT(*)', 'WHERE `pid`='.$pid, 'count'));
...
/*
 * 
DROP TABLE IF EXISTS `newsfiles`;
CREATE TABLE IF NOT EXISTS `newsfiles` (
