...
if(!$arrPageData['moduleRootID']){
    $arrPageData['errors'][]    = sprintf(ADMIN_MODULE_ID_ERROR, BLOGNEWS, $arrPageData['module']);
    $arrPageData['module']      = 'common/module_messages';
    $arrPageData['moduleTitle'] = BLOGNEWS;
    return;
} else {
    foreach($arAcceptLangs as $ln){
        $dbTable = replaceLang($ln, BLOG_TABLE);
        if(!$DB->isSetDBTable($dbTable)){
            $arrPageData['errors'][]    = sprintf(ADMIN_MODULE_TABLE_ERROR, BLOGNEWS, $arrPageData['module'], $dbTable);
            $arrPageData['module']      = 'common/module_messages';
            $arrPageData['moduleTitle'] = BLOGNEWS;
            return;
        }
...
$arCidCntItems = getItemsCountInCategories('cid', 'count', BLOG_TABLE, '`cid`,COUNT(`cid`) as count', 'WHERE `active`=1 GROUP BY `cid`');
...
$arrPageData['headTitle']	= BLOGNEWS.$arrPageData['seo_separator'].$arrPageData['headTitle'];
...
$arrPageData['images_params']	= array(array('', 800, 600), array("small_", 320, 240), array('tiny_', 120, 90)); // array(array(addname, width, height), array(addname, width, height)[, ..]);
$arrPageData['filters']		= getComplexRowItems(TAGS_TABLE, '*');
...
if($task=='reorderItems' && !empty($_POST)) {
    $result = reorderItems($_POST['arOrder'], 'order', 'id', BLOG_TABLE);
    if($result===true) {
...
elseif($itemID && $task=='deleteItem') {
    unlinkImageLangsSynchronize($itemID, BLOG_TABLE, $arrPageData['files_path'], $arrPageData['images_params']);
    // operation with linked categories
    if(getValueFromDB(BLOG_TOCAT_TABLE, 'COUNT(*)', 'WHERE pid='.(int)$itemID, 'cnt') > 0) {
	deleteRecords(BLOG_TOCAT_TABLE, 'WHERE pid='.(int)$itemID);
    }
    // operation with linked files
    if(getValueFromDB(BLOGFILES_TABLE, 'COUNT(*)', 'WHERE pid='.(int)$itemID, 'cnt') > 0) {
	deleteRecords(BLOGFILES_TABLE, 'WHERE pid='.(int)$itemID);
    }
    removeDir($arrPageData['files_path'].DS.$itemID);
    // operation with linked comments
    deleteRecords(COMMENTS_TABLE, 'WHERE `pid`='.(int)$itemID.' AND `module`="'.$arrPageData['module'].'"');
    // operation with linked tags
    deleteRecords(TAGS_TO_ENTRY_TABLE, 'WHERE `pid`='.(int)$itemID.' AND `module`="'.$arrPageData['module'].'"');
    $result = deleteDBLangsSync(BLOG_TABLE, ' WHERE id='.(int)$itemID);
    if (!$result) {
...
elseif($itemID && $task=='publishItem' && isset($_GET['status'])) {
    $result = updateRecords(BLOG_TABLE, "`active`='{$_GET['status']}'", 'WHERE `id`='.(int)$itemID);
    if ($result===false) {
...
elseif(!empty($_POST) && ($task=='addItem' || $task=='editItem')) {
    ...
    if($Validator->foundErrors()==0){
        ...
        if(getValueFromDB(BLOG_TABLE, 'COUNT(*)', "WHERE `seo_path`='{$_POST['seo_path']}'".($cid ? ' AND `cid`='.$cid : '').($itemID ? ' AND `id`<>'.$itemID : ''), 'count')>0) {
    ...
    if ($Validator->foundErrors()) {
        ...
        $arPostData = $_POST;
        $arPostData['image'] = imageManipulation($itemID, BLOG_TABLE, $arrPageData['files_url'], $arrPageData['images_params']);
        ...
        $result = $DB->postToDB($arPostData, BLOG_TABLE, $conditions,  $arUnusedKeys, $query_type, ($itemID ? false : true));
        if($result){
	    // operation with category linking
	    deleteRecords(BLOG_TOCAT_TABLE, 'WHERE `pid`='.(int)$itemID);
	    if(isset($arPostData['arCategories']) && !empty($arPostData['arCategories'])) {
		foreach ($arPostData['arCategories'] as $key => $value) {
			...
			$DB->postToDB($cat, BLOG_TOCAT_TABLE);
		    }
		    unset($cat);
		}
	    }
	    // operation with tags linking
	    if(isset($arPostData['filters']) && !empty($arPostData['filters'])) {
		foreach ($arPostData['filters'] as $key => $value) {
		    $filter = array(
			    'tid'	=> $value,
			    'pid'	=> $itemID,
			    'module'	=> $arrPageData['module']
		    );
		    if(!empty($filter['tid']) && !empty($filter['pid']) && !empty($filter['module'])) {
			$DB->postToDB($filter, TAGS_TO_ENTRY_TABLE);
		    }
		    unset($filter);
		}
	    }
...
if($task=='addItem' || $task=='editItem'){
    if(!$itemID){
        $item = array_combine_multi($DB->getTableColumnsNames(BLOG_TABLE), '');
        $item['order']  = getMaxPosition($cid, 'order', 'cid', BLOG_TABLE);
        $item['active'] = 1;
	...
	$item['filters'] = array();
        ...
        $query = "SELECT * FROM ".BLOG_TABLE." WHERE id = $itemID LIMIT 1";
	    ...
	    $item['arCategories'] = getArrValueFromDB(BLOG_TOCAT_TABLE, 'cid', 'WHERE `pid`='.(int)$itemID);
	    $item['comments']	  = intval(getValueFromDB(COMMENTS_TABLE, 'COUNT(*)', 'WHERE `pid`='.(int)$itemID.' AND `module`="'.$arrPageData['module'].'"', 'cnt'));
	    $item['filters']	  = getArrValueFromDB(TAGS_TO_ENTRY_TABLE, 'tid', 'WHERE `pid`='.(int)$itemID.' AND `module`="'.$arrPageData['module'].'"');
        }
    }
    ...
    if($cid > 0) {
	$where = 'WHERE t.id IN(SELECT c.pid FROM '.BLOG_TOCAT_TABLE.' c WHERE c.cid='.$cid.') ';
    } else {
	$where = "";
    }
    // Total pages and Pager
    $arrPageData['total_items'] = intval(getValueFromDB(BLOG_TABLE." t", 'COUNT(*)', $where, 'count'));
    ...
    $query  = "SELECT * FROM `".BLOG_TABLE."` t $where $order $limit";
    $result = mysql_query($query);
    if(!$result) {
	$arrPageData['errors'][] = "SELECT OPERATIONS: " . mysql_error();
    } else {
	    ...
	    $row['comments']	= intval(getValueFromDB(COMMENTS_TABLE, 'COUNT(*)', 'WHERE `pid`='.(int)$row['id'].' AND `module`="'.$arrPageData['module'].'"', 'cnt'));
            $items[]		= $row;
        }
    }
}
...
CREATE TABLE IF NOT EXISTS `ru_blog` (
 ...
 * 
DROP TABLE IF EXISTS `tags_to_entry`;
