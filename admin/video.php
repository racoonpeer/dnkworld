...
$arrPageData['cid']           = $cid = ((!$cid && !empty($categoryTree)) ? $categoryTree[0]['id'] : (!$cid ? $arrPageData['moduleRootID'] : $cid));
...
$arrPageData['def_img_param'] = array('w'=>640, 'h'=>302);
$arrPageData['images_params'] = array(array("small_", 424, 200),array("middle_", 452, 213)); // array(array(addname, width, height), array(addname, width, height)[, ..]);
...
if($task=='reorderItems' && !empty($_POST)) {
    $result = reorderItems($_POST['arOrder'], 'order', 'id', VIDEOS_TABLE);
...
elseif(!empty($_POST) && ($task=='addItem' || $task=='editItem')) {
    ...
    if($Validator->foundErrors()==0){
        ...
        if(getValueFromDB(VIDEOS_TABLE, 'COUNT(*)', "WHERE `seo_path`='{$_POST['seo_path']}'".($cid ? ' AND `cid`='.$cid : '').($itemID ? ' AND `id`<>'.$itemID : ''), 'count')>0)
...
if($task=='addItem' || $task=='editItem'){
    ...
    $query  = "SELECT t.* FROM `".VIDEOS_TABLE."` t $where $order $limit";
    $result = mysql_query($query);
