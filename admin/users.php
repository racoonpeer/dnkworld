...
elseif($itemID && $task=='activateItem' && isset($_GET['status'])) {
    ...
    if($result && $_GET['status'] && !empty($arUser)) {
        // Send Activation email to user
        $arUser['username'] = $arUser['company']; //trim(trim("{$arUser['firstname']} {$arUser['middlename']}")." {$arUser['surname']}");
        $arUser['sitename'] = $objSettingsInfo->websiteName;
        $arUser['links']['recovery'] = 'http://'.$_SERVER['SERVER_NAME'].$UrlWL->buildItemUrl($arrModules['recovery']);
        $arUser['links']['login'] = 'http://'.$_SERVER['SERVER_NAME'].$UrlWL->buildItemUrl($arrModules['authorize']);
...
elseif($itemID && $task=='publishItem' && isset($_GET['status'])) {
    ...
    elseif($result)     $arrPageData['messages'][] = 'Новое состояние сохранено!';
}
...
elseif(!empty($_POST) && ($task=='addItem' || $task=='editItem')) {
    ...
    if($itemID && $objUserInfo->type != 'Administrator')
    ...
    if(!empty($_POST['login']) && $_POST['login'] != $_POST['old_login']){
    ...
    if ($Validator->foundErrors()) {
        ...
        if($result){
            ...
            Redirect($arrPageData['admin_url'].$arrPageData['page_url'].(isset($_POST['submit_add']) ? '&task=addItem' : ((isset($_POST['submit_apply']) && $itemID) ? '&task=editItem&itemID='.$itemID : '')) );
        } else {
...
// //////////////////////////////////\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
// New User List Data
$query = "SELECT * FROM ".USERS_TABLE." WHERE `isnew`=1 ORDER BY `created` DESC";
$result = mysql_query($query);
