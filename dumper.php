...
\***************************************************************************/
$is_dumper = session_id();
if(!empty($is_dumper)){
       defined('WEBlife') or die("Нельзя запускать без админки!!!");
} else define('WEBlife', 1); //Set flag that this is a parent file
$is_dumper = @intval($_SESSION['is_dumper']);

require_once('include/classes/Cookie.php');         // 1. Include Cookie class file
$dCookie = new CCookie();

require_once('include/system/SystemComponent.php');
class DumperDB extends SystemComponent {
    public function getDumperDB() {
        return parent::getDBSettings();
    }
}

$dDB = DumperDB::getDumperDB();
// DB params
define('DBHOST',  $dDB['dbhost']);
define('DBNAMES', $dDB['dbname']);
...
define('PATH', 'backup/');
// Ссылка для скачивания файлов с закрытой директории по HTTP протоколу.
define('URL',  '/interactive/ajax.php?zone='.($is_dumper?'admin&uid='.$objUserInfo->id:'site').'&action=dbFileBackUpDownload&file=');
// Ссылка для выхода из Dumpera.
define('EXIT_URL',  $is_dumper ? '/admin.php?reload' : '/'.basename(__FILE__).'?reload');
...
if($is_dumper) {
    $userdb     = $dDB['dbusername'];
    $passdb     = $dDB['dbpassword'];
    $auth       = @mysql_connect(DBHOST, $userdb, $passdb) ? 1 : 0;
    $admin_url  = 'admin.php?module=mysqldumper';
} elseif (!empty($_POST['login']) && isset($_POST['pass'])) {
    $login = addslashes(stripslashes($_POST['login']));
    $pass  = addslashes(stripslashes($_POST['pass']));
    if (@mysql_connect(DBHOST, $login, $pass)) {
        $dCookie->add(DCOOKIE, base64_encode("SKD101:{$login}:{$pass}"), 0);
        $dCookie->process();
        header("Location: dumper.php");
        mysql_close();
        exit;
    } else $error = '#' . mysql_errno() . ': ' . mysql_error();
} elseif ($dCookie->getCookie(DCOOKIE)!='') {
    $user = explode(":", base64_decode($dCookie->getCookie(DCOOKIE)));
...
if (!$auth || (isset($_SERVER['QUERY_STRING']) && $_SERVER['QUERY_STRING'] == 'reload')) {
    if(!$is_dumper) $dCookie->del(DCOOKIE);
    $dCookie->process();
...
class dumper {
    ...
    function backup() {
        ...
        $buttons = "<A ID=save HREF='' STYLE='display: none;' target='_blank'>Скачать файл</A> &nbsp; <INPUT ID=back TYPE=button VALUE='Вернуться' DISABLED onClick=\"history.back();\">";
        ...
        echo "<SCRIPT>with (document.getElementById('save')) {style.display = ''; innerHTML = 'Скачать файл ({$filesize})'; href = '".URL.urlencode($this->filename)."'; }document.getElementById('back').disabled = 0;</SCRIPT>";
    }
    ...
    function main() {
        ...
        $buttons = "<INPUT TYPE=submit VALUE=Применить><INPUT TYPE=button VALUE=Выход onClick=\"location.href = '".EXIT_URL."'\">";
...
function tpl_page($content = '', $buttons = ''){
    global $is_dumper;
    return $is_dumper ? tpl_page_from_admin($content, $buttons) : tpl_page_base($content, $buttons);
}

        
// Шаблоны

function tpl_page_base($content = '', $buttons = ''){
return <<<HTML
...
<B><A HREF=http://sypex.net/products/dumper/ STYLE="color: white; text-decoration: none;" target="_blank">Sypex Dumper Lite 1.0.8</A></B></TD>
</TR>
<TR>
...
function tpl_page_from_admin($content = '', $buttons = '') {
    global $admin_url;
    return <<<HTML
<STYLE TYPE="TEXT/CSS">
<!--
...
<TR>
<FORM NAME=skb METHOD=POST ACTION={$admin_url}>
<TD VALIGN=TOP BGCOLOR=#F4F3EE STYLE="FILTER: progid:DXImageTransform.Microsoft.Gradient(gradientType=0,startColorStr=#FCFBFE,endColorStr=#F4F3EE); padding: 8px 8px;">
    {$content}
<TABLE WIDTH=100% BORDER=0 CELLSPACING=0 CELLPADDING=2>
<TR>
